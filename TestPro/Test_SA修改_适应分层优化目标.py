...
if df is not None:
    ...
    #
    # # 目标函数2：最小化出库能耗与时间
    # def minimize_outbound_energy_time_with_batch(particle_positions, plates, heights, batches):
    #     total_time_energy = 0
    #
    #     # 按批次将钢板索引分配
    ...
    class PSO_with_Batch:
        ...
        def save_metrics(self, time_elapsed):
            ...
            file_name = f"comparison_performance_pso.csv"
    ...
    class GA_with_Batch:
        ...
        def optimize(self):
            ...
            fitness_evaluations = None  # 遵照PSO规则，适应度函数评估次数为None
    ...
    class SA_with_Batch:
        def __init__(self, initial_temperature, cooling_rate, min_temperature, max_iterations, lambda_1, lambda_2,
                     lambda_3, lambda_4, num_positions, dataset_name, Dki):  # 增加 Dki 参数
        ...
        def evaluate(self, position):
            ...
            try:
                ...
                # 调用目标函数2：最小化出库能耗与时间
                energy_time_penalty = minimize_outbound_energy_time_with_batch(
                    particle_positions=position,
                    plates=plates,
                    heights=temp_heights,
                    batches=batches,
                ...
                # 调用目标函数3：最大化库存均衡度
                balance_penalty = maximize_inventory_balance_v2(
                    particle_positions=position,
                    plates=plates
                )

                # 调用目标函数4：空间利用率最大化
                space_utilization = maximize_space_utilization_v3(
                    particle_positions=position,
                    plates=plates,
                    Dki=self.Dki  # 使用 self.Dki
                )

                # 综合优化目标，使用给定的 lambda 参数加权求和
                score = (self.lambda_1 * combined_movement_turnover_penalty +
                         self.lambda_2 * energy_time_penalty +
                         self.lambda_3 * balance_penalty -
                         self.lambda_4 * space_utilization)
